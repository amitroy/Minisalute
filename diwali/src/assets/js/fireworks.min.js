...
self.canvasLoop=function(){requestAnimFrame(self.canvasLoop,self.canvas);self.updateDelta();self.ctx.globalCompositeOperation='destination-out';self.ctx.fillStyle='rgba(0,0,0,'+self.clearAlpha/100+')';self.ctx.fillRect(0,0,self.cw,self.ch);self.ctx.globalCompositeOperation='lighter';self.updateFireworks();self.updateParticles();self.drawFireworks();self.drawParticles()};self.init();setInterval(function(){if(window_focus){self.currentHue=rand(self.hueMin,self.hueMax);self.lineWidth=1;self.partCount=80
self.partFriction=rand(4,8);self.fworkAccel=rand(1,4);self.partWind=rand(50,70);self.partGravity=rand(1,5);self.fireworks.push(new Firework(self.cw/2,self.ch,rand(50,self.cw-50),rand(50,self.ch/2)-50));self.currentHue=rand(self.hueMin,self.hueMax);self.partFriction=rand(4,8);self.fworkAccel=rand(1,4);self.partWind=rand(50,70);self.partGravity=rand(1,5);self.fireworks.push(new Firework(self.cw/2,self.ch,rand(50,self.cw-50),rand(50,self.ch/2)-50));$("#rotate").effect('bounce',3000).effect('pulsate')}},2000)}
var fworks=new Fireworks();(function($){$.fn.extend({rotaterator:function(options){var defaults={fadeSpeed:500,pauseSpeed:100,child:null};var options=$.extend(defaults,options);return this.each(function(){var o=options;var obj=$(this);var items=$(obj.children(),obj);items.each(function(){$(this).hide()})
if(!o.child){var next=$(obj).children(':first')}else{var next=o.child}
$(next).fadeIn(o.fadeSpeed,function(){$(next).delay(o.pauseSpeed).fadeOut(o.fadeSpeed,function(){var next=$(this).next();if(next.length==0){next=$(obj).children(':first')}
$(obj).rotaterator({child:next,fadeSpeed:o.fadeSpeed,pauseSpeed:o.pauseSpeed})})})})}})})(jQuery);$(document).ready(function(){$('#rotate').rotaterator({fadeSpeed:1000,pauseSpeed:5000})});function launchFullscreen(element){if(element.requestFullscreen){element.requestFullscreen()}else if(element.mozRequestFullScreen){element.mozRequestFullScreen()}else if(element.webkitRequestFullscreen){element.webkitRequestFullscreen()}else if(element.msRequestFullscreen){element.msRequestFullscreen()}}
function exitFullscreen(){if(document.exitFullscreen){document.exitFullscreen()}else if(document.mozCancelFullScreen){document.mozCancelFullScreen()}else if(document.webkitExitFullscreen){document.webkitExitFullscreen()}}
$(window).focus(function(){window_focus=!0});$(window).focusout(function(){window_focus=!1});var iOS=/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream;if(iOS){$('.minsalbtnpair').hide()};$(".container").click(function(){window_focus=!0});$(function(){$("#chkbox input").checkboxradio()});$('#btnCreate').click(function(){var domain="http://diwali.minisalute.com/h/";var whatsappurl="whatsapp://send?text=💥 🎊 Wishing you a Very Happy and Prosperous Diwali 🎉🎊. Click the link below for an interactive experience.";var rawurl=null;var cookedurl=null;var n=$('input[name=fromname]').val();var cr=$('input[name=color-radio]:checked').val();var mr=$('input[name=message-radio]:checked').val();var ir=$('input[name=icon-radio]:checked').val();if(n==null||n==''){rawurl=cr+'|'+mr+'|'+ir+'|'+'}'}else{rawurl=cr+'|'+mr+'|'+ir+'|'+n}
cookedurl=domain+btoa(rawurl);$('.shareurl').val(cookedurl);$('#testshareurl').attr("action",cookedurl);$('#whatsappshare').attr("href",whatsappurl+cookedurl)
$('#btnTest').show();$('.shareurl').show();$('#sharebuttoncontainer').show()})